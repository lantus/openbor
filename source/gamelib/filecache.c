...
int find_least_useful_cacheblock(void) {
	...
	for(i = 0; i < filecache_blocks; i++) {
		...
		if(vfd < max_vfds)
			continue;
		if((leastcacheblock < 0) || (cacheblock_mru[i] < cacheblock_mru[leastcacheblock])) {
			leastcacheblock = i;
		}
	}
...
static int get_vfd_blocks_available(int vfd) {
	...
	for(i = ptr; i < max; i++)
		if(where_is_this_pakblock_cached[i] >= filecache_blocks)
			break;
	return i - ptr;
}
...
void filecache_process(void) {
	...
	if(pakblock_read >= 0 && cacheblock_read >= 0) {
...
#endif
		{
			...
			disCcWarns =
			    read(real_pakfd, (char *) filecache + (cacheblock_read * filecache_blocksize),
				 filecache_blocksize);
#endif
		}
	}
