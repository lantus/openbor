...
static int readbmp(unsigned char *buf, unsigned char *pal, int maxwidth, int maxheight) {
	...
	if(buf) {
		...
		while(y < maxheight && y < bmp_header.ysize) {
			...
			seekpackfile(handle, bmp_header.picstart + ((bmp_header.ysize - y - 1) * bmp_header.xsize),
				     SEEK_SET);
	...
	if(pal && (linebuffer = (unsigned char *) malloc(1024))) {
		...
		if(pb == 512)	// 16bit 565
		{
			for(s = 0, d = 0; s < 1024; s += 4, d += 2) {
				*(unsigned short *) (pal + d) =
				    colour16(linebuffer[s + 2], linebuffer[s + 1], linebuffer[s]);
			}
...
static int readpng(unsigned char *buf, unsigned char *pal, int maxwidth, int maxheight) {
	...
	if(pal) {
		...
		if(png_get_PLTE(png_ptr, info_ptr, &png_pal_ptr, &png_pal_num) != PNG_INFO_PLTE || png_pal_ptr == NULL) {
			return 0;
		}
		...
		if(pb == 512)	// 16bit 565
		{
			for(i = 0, j = 0; i < 512 && j < png_pal_num; i += 2, j++) {
				*(unsigned short *) (pal + i) =
				    colour16(png_pal_ptr[j].red, png_pal_ptr[j].green, png_pal_ptr[j].blue);
			}
		...
		} else if(pb == 1024)	// 32bit
		{
			...
			for(i = 0, j = 0; i < 1024 && j < png_pal_num; i += 4, j++) {
				*(unsigned *) (pal + i) =
				    colour32(png_pal_ptr[j].red, png_pal_ptr[j].green, png_pal_ptr[j].blue);
			}
		}
	}
	return 1;
...
static int decodegifblock(int handle, unsigned char *buf, int width, int height, unsigned char bits,
			  gifblockstruct * gb) {
	short bits2;
	short codesize;
	short codesize2;
...
int loadscreen(char *filename, char *packfile, unsigned char *pal, int format, s_screen ** screen) {
	...
	if(!openimage(filename, packfile))
		return 0;
	if(!(*screen)
	   || ((*screen)->width != res[0] && (*screen)->height != res[1] && (*screen)->pixelformat != format)) {
