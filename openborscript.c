...
void mapstrings_systemvariant(ScriptVariant ** varlist, int paramCount) {
	...
	MAPSTRINGS(varlist[0], proplist, _sv_the_end, "openborvariant: System variable name not found: '%s'\n");
}
...
void mapstrings_changesystemvariant(ScriptVariant ** varlist, int paramCount) {
	...
	MAPSTRINGS(varlist[0], proplist, _csv_the_end, "changeopenborvariant: System variable name not found: '%s'\n");
}
...
HRESULT openbor_drawstringtoscreen(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawstring_error:
	printf
	    ("Function needs a valid screen handle, 3 integers and a string value: drawstringtoscreen(screen, int font, value)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawbox(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	spriteq_add_box((int) value[0], (int) value[1], (int) value[2], (int) value[3], (int) value[4], (int) value[5],
			l);
	...
	drawbox_error:
	printf
	    ("Function requires 6 integer values: drawbox(int x, int y, int width, int height, int z, int color, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawboxtoscreen(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawbox_error:
	printf
	    ("Function requires a screen handle and 5 integer values, 7th integer value is optional: drawboxtoscreen(screen, int x, int y, int width, int height, int color, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawline(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	spriteq_add_line((int) value[0], (int) value[1], (int) value[2], (int) value[3], (int) value[4], (int) value[5],
			 l);
	...
	drawline_error:
	printf
	    ("Function requires 6 integer values, 7th integer value is optional: drawline(int x1, int y1, int x2, int y2, int z, int color, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawlinetoscreen(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawline_error:
	printf
	    ("Function requires a screen handle and 5 integer values, 7th integer value is optional: drawlinetoscreen(screen, int x1, int y1, int x2, int y2, int color, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawspritetoscreen(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawsprite_error:
	printf
	    ("Function requires a valid sprite handle, a valid screen handle and 2 integer values: drawspritetoscreen(sprite, screen, int x, int y)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawdot(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawdot_error:
	printf
	    ("Function requires 4 integer values, 5th integer value is optional: drawdot(int x, int y, int z, int color, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawdottoscreen(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawdot_error:
	printf
	    ("Function requires a screen handle and 3 integer values, 5th integer value is optional: dottoscreen(screen, int x, int y, int color, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_drawscreen(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	drawscreen_error:
	printf
	    ("Function requires a screen handle and 3 integer values, 5th integer value is optional: drawscreen(screen, int x, int y, int z, int lut)\n");
	return E_FAIL;
}
...
HRESULT openbor_getentityvar(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(arg->vt == VT_EMPTY)
		...
		printf
		    ("Function's 1st argument must be a valid entity handle value or empty value: getentityvar(entity, int index)\n");
		*pretvar = NULL;
		return E_FAIL;
	}
	if(!ent || !ent->entvars)
...
HRESULT openbor_setentityvar(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(arg->vt == VT_EMPTY)
		...
		printf
		    ("Function's 1st argument must be a valid entity handle value or empty value: setentityvar(entity, int index, var)\n");
		*pretvar = NULL;
		return E_FAIL;
	}
	if(!ent || !ent->entvars)
...
HRESULT openbor_strlength(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(varlist[0]->vt != VT_STR) {
		printf
		    ("Error, strlength({string}): Invalid or missing parameter. Strlength must be passed a valid {string}.\n");
	}
		...
		case _gep_icon:
			{
				...
				if(arg->vt != VT_INTEGER) {
					printf
					    ("Error, getentityproperty({ent}, 'icon', {type}): {type} parameter missing or invalid. \n {type}: \n 0 = Icon \n 1 = Icondie \n 2 = Iconget \n 3 = Iconmphigh \n 4 = Iconmphalf \n 5 = Iconmplow \n 6 = Iconpain \n 7 = Iconw \n 8 = Iconx \n 9 = Icony \n");
					return E_FAIL;
				}
		...
		case _gep_energycost:
			{
				if(paramCount != 4	//Validate parameter count and types.
				   || varlist[2]->vt != VT_INTEGER || varlist[3]->vt != VT_INTEGER) {
					printf
		...
		case _gep_komap:
			{
				...
				if(arg->vt != VT_INTEGER) {
					printf
					    ("You must provide a Komap parameter index: \n 0 = Komap \n 1 = Komap type \n");
					return E_FAIL;
				}
		...
		case _gep_defense:
			{
				...
				switch (arg->lVal) {
					case _gep_defense_factor:
						{
							(*pretvar)->dblVal =
							    (float) ent->modeldata.defense_factors[(int) ltemp];
							break;
						}
		...
		case _gep_bbox:
			{
				...
				if(varlist[2]->vt != VT_INTEGER
				   ...
				   || varlist[4]->vt != VT_INTEGER || varlist[5]->vt != VT_INTEGER) {
					printf
		...
		case _gep_range:
			{
				...
				if(varlist[2]->vt != VT_INTEGER || varlist[3]->vt != VT_INTEGER) {
					printf
		...
		case _gep_attack:
			{
				...
				if(arg->vt != VT_INTEGER) {
					printf
					    ("Error, getentityproperty({ent}, 'attack', {sub property}, {index}, {animation}, {frame}): You must give a string name for {sub property}.\n");
					return E_FAIL;
				}
				ltemp = arg->lVal;
		...
		case _gep_flash:
			{
				...
				if(paramCount < 3 || arg->vt != VT_INTEGER) {
					printf
					    ("Error, getentityproperty({ent}, 'flash', {argument}): Missing or invalid parameter. \n {argument}: \n 0 = Flash \n 1 = Noatflash \n 2=bflash \n");
					return E_FAIL;
				}
		...
		case _gep_stats:
			{
				...
				if(arg->vt != VT_INTEGER || arg1->vt != VT_INTEGER) {
					printf
					    ("Incorrect parameters: getentityproperty({ent}, 'stats', {type}, {index}) \n {type}: \n 0 = Model. \n 1 = Entity. \n");
					return E_FAIL;
				}
		...
		case _gep_staydown:
			{
				...
				if(arg->vt != VT_INTEGER) {
					printf
					    ("Incorrect parameters: getentityproperty({ent}, 'staydown', {type}) \n {type}: \n 0 = Staydown. \n 1 = Staydownatk. \n");
					return E_FAIL;
				}
...
void mapstrings_changeentityproperty(ScriptVariant ** varlist, int paramCount) {
	...
	if((varlist[1]->vt == VT_INTEGER) && (varlist[1]->lVal == _cep_think)) {
		MAPSTRINGS(varlist[2], proplist_think, _cep_th_the_end, "Set '%s' is not supported by 'think'.\n");
	}
	...
	if((varlist[1]->vt == VT_INTEGER) &&
	   ((varlist[1]->lVal == _cep_hostile) || (varlist[1]->lVal == _cep_candamage)
	    || (varlist[1]->lVal == _cep_projectilehit))) {
...
HRESULT openbor_changeentityproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	switch (propind) {
		case _cep_takeaction:
			{
				if(varlist[2]->vt == VT_STR) {	// not a known action; if it were it would been mapped by mapstrings
					ent->takeaction = NULL;
					break;
		...
		case _cep_defaultname:
			{
				...
				if(!tempmodel) {
					printf
					    ("Use must give an existing model's name for entity's default model name.\n");
					goto changeentityproperty_error;
				}
		...
		case _cep_combostep:
			{
				if(paramCount >= 4 && SUCCEEDED(ScriptVariant_IntegerValue(varlist[2], &ltemp))) {
					(*pretvar)->lVal = (LONG) 1;
				}
				if((*pretvar)->lVal == (LONG) 1) {
					if(SUCCEEDED(ScriptVariant_IntegerValue(varlist[3], &ltemp2)))
...
HRESULT openbor_getplayerproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(arg->vt != VT_INTEGER) {
		if(arg->vt != VT_STR)
			printf
			    ("Function call getplayerproperty has invalid propery name parameter, it must be a string value.\n");
		*pretvar = NULL;
		return E_FAIL;
	}
	prop = arg->lVal;
...
HRESULT openbor_openfilestream(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(location) {
		...
		if(level->filestreams[level->numfilestreams].buf == NULL)
			return E_FAIL;
		disCcWarns = fread(level->filestreams[level->numfilestreams].buf, 1, size, handle);
	} else
	    if(buffer_pakfile
	       (filename, &level->filestreams[level->numfilestreams].buf,
		&level->filestreams[level->numfilestreams].size) != 1) {
...
HRESULT openbor_getfilestreamline(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	while(level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos + length]
	      && level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos + length] != '\n'
	      && level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos + length] != '\r')
		++length;
	if(length >= MAX_STR_VAR_LEN)
		strncpy(StrCache_Get((*pretvar)->strVal),
			(char *) (level->filestreams[filestreamindex].buf + level->filestreams[filestreamindex].pos),
			MAX_STR_VAR_LEN);
	else {
		strncpy(line,
			(char *) (level->filestreams[filestreamindex].buf + level->filestreams[filestreamindex].pos),
...
HRESULT openbor_getfilestreamargument(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(varlist[2]->vt != VT_STR) {
		printf
		    ("You must give a string value specifying what kind of value you want the argument converted to.\n");
		return E_FAIL;
	}
	...
	if(stricmp(argtype, "string") == 0) {
		ScriptVariant_ChangeType(*pretvar, VT_STR);
		strncpy(StrCache_Get((*pretvar)->strVal),
			(char *) findarg(level->filestreams[filestreamindex].buf +
					 level->filestreams[filestreamindex].pos, argument), MAX_STR_VAR_LEN);
...
HRESULT openbor_filestreamnextline(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	while(level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos]
	      && level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos] != '\n'
	      && level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos] != '\r')
		++level->filestreams[filestreamindex].pos;
	while(level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos] == '\n'
	      || level->filestreams[filestreamindex].buf[level->filestreams[filestreamindex].pos] == '\r')
		++level->filestreams[filestreamindex].pos;
...
HRESULT openbor_savefilestream(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(handle == NULL)
		return E_FAIL;
	disCcWarns =
	    fwrite(level->filestreams[filestreamindex].buf, 1, strlen(level->filestreams[filestreamindex].buf), handle);
...
HRESULT openbor_checkrange(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	checkrange_error:
	printf
	    ("Function needs at least 2 valid entity handles, the third parameter is optional: checkrange(entity, target, int animnum)\n");
	*pretvar = NULL;
	return E_FAIL;
}
...
void mapstrings_setspawnentry(ScriptVariant ** varlist, int paramCount) {
	...
	MAPSTRINGS(varlist[0], proplist, _sse_the_end, "Property name '%s' is not supported by setspawnentry.\n");
}
...
HRESULT openbor_setspawnentry(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	switch (prop) {
		case _sse_name:
			if(arg->vt != VT_STR) {
				printf
				    ("You must use a string value for spawn entry's name property: function setspawnentry.\n");
...
HRESULT openbor_projectile(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	switch ((int) value[5]) {
		...
		case 0:
			ent =
			    knife_spawn(name, -1, (float) value[0], (float) value[1], (float) value[2], (int) value[3],
					(int) value[4], (int) value[6]);
			break;
		case 1:
			ent =
			    bomb_spawn(name, -1, (float) value[0], (float) value[1], (float) value[2], (int) value[3],
				       (int) value[6]);
			break;
	}
	...
	projectile_error:
	printf
	    ("Function requires 8 values: entity * projectile(char *name, float x, float z, float a, int direction, int type, int ptype, int map)\n");
	return E_FAIL;
}

#define SAMPLE_GO samples.go
#define SAMPLE_BEAT samples.beat
#define SAMPLE_BLOCK samples.block
#define SAMPLE_INDIRECT samples.indirect
#define SAMPLE_GET samples.get
#define SAMPLE_GET2 samples.get2
#define SAMPLE_FALL samples.fall
#define SAMPLE_JUMP samples.jump
#define SAMPLE_PUNCH samples.punch
#define SAMPLE_1UP samples.oneup
#define SAMPLE_TIMEOVER samples.timeover
#define SAMPLE_BEEP samples.beep
#define SAMPLE_BEEP2 samples.beep2
#define SAMPLE_BIKE samples.bike


// ===== openborconstant =====

#define ICMPCONST(x) \
	case CMD_SCRIPT_CONSTANT_ ##x: \
	value = x; break;
void mapstrings_transconst(ScriptVariant ** varlist, int paramCount) {
	...
	if(varlist[0]->vt == VT_STR) {
		...
		cmd = getScriptConstantsCommand(scriptConstantsCommandList, constname);
		vartype = ScriptVariant_GetType(varlist[0]);
		...
		switch(cmd) {
			...
			default:
				ScriptVariant_ChangeType(varlist[0], vartype);
				return;
		}
		
		varlist[0]->lVal = value;
	}
}
...
void mapstrings_playerkeys(ScriptVariant ** varlist, int paramCount) {
	...
	for(i = 2; i < paramCount; i++) {
		MAPSTRINGS(varlist[i], proplist, _pk_the_end, "Button name '%s' is not supported by playerkeys.");
	}
}
...
HRESULT openbor_fademusic(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	fademusic_error:
	printf
	    ("Function requires 1 value, with an optional 3 for music triggering: fademusic_error(float fade, char name, int loop, unsigned long offset)\n");
	return E_FAIL;
}
	...
	*pretvar = NULL;
	...
	if((int) value[0] < 0) {
		printf("Invalid Id for playsample(id=%d, priority=%d, lvolume=%d, rvolume=%d, speed=%d, loop=%d)\n",
		       (int) value[0], (unsigned int) value[1], (int) value[2], (int) value[3], (unsigned int) value[4],
		       (int) value[5]);
		return E_FAIL;
	}
	if((int) value[5])
		sound_loop_sample((int) value[0], (unsigned int) value[1], (int) value[2], (int) value[3],
				  (unsigned int) value[4]);
	else
		sound_play_sample((int) value[0], (unsigned int) value[1], (int) value[2], (int) value[3],
				  (unsigned int) value[4]);
	return S_OK;
	...
	printf
	    ("Function requires 6 integer values: playsample(int id, unsigned int priority, int lvolume, int rvolume, unsigned int speed, int loop)\n");
	return E_FAIL;
}
...
HRESULT openbor_loadsample(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	loadsample_error:
	printf
	    ("Function requires 1 string value and optional log value: loadsample(string {filename} integer {log})\n");
	*pretvar = NULL;
	return E_FAIL;
}
...
HRESULT openbor_changeshadowcolor(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	printf
	    ("Function requires at least 1 integer value, the 2nd integer parameter is optional: changepalette(int colorindex, int alpha)\n");
	return E_FAIL;
}
...
HRESULT openbor_changetextobjproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(FAILED(ScriptVariant_IntegerValue(varlist[0], &ind))) {
		printf
		    ("Function's 1st argument must be a numeric value: changetextobjproperty(int index, \"property\", value)\n");
		goto changetextobjproperty_error;
	}
		...
		case _ctop_text:
			{
				...
				strncpy(level->textobjs[ind].text, (char *) StrCache_Get(varlist[2]->strVal),
					MAX_STR_VAR_LEN);
...
HRESULT openbor_settextobj(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(FAILED(ScriptVariant_IntegerValue(varlist[0], &ind))) {
		printf
		    ("Function's 1st argument must be a numeric value: settextobj(int index, int x, int y, int font, int z, char text, int time {optional})\n");
		goto settextobj_error;
	}
...
HRESULT openbor_getbglayerproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(FAILED(ScriptVariant_IntegerValue(varlist[0], &ind))) {
		printf
		    ("Function's 1st argument must be a numeric value: getbglayerproperty(int index, \"property\", value)\n");
		goto getbglayerproperty_error;
	}
...
HRESULT openbor_changebglayerproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(FAILED(ScriptVariant_IntegerValue(varlist[0], &ind))) {
		printf
		    ("Function's 1st argument must be a numeric value: changebglayerproperty(int index, \"property\", value)\n");
		goto changebglayerproperty_error;
	}
...
HRESULT openbor_getfglayerproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(FAILED(ScriptVariant_IntegerValue(varlist[0], &ind))) {
		printf
		    ("Function's 1st argument must be a numeric value: getfglayerproperty(int index, \"property\", value)\n");
		goto getfglayerproperty_error;
	}
...
HRESULT openbor_changefglayerproperty(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	if(FAILED(ScriptVariant_IntegerValue(varlist[0], &ind))) {
		printf
		    ("Function's 1st argument must be a numeric value: changefglayerproperty(int index, \"property\", value)\n");
		goto changefglayerproperty_error;
	}
...
HRESULT openbor_jumptobranch(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	jumptobranch_error:
	printf
	    ("Function requires 1 string value, the second argument is optional(int): jumptobranch(name, immediate)\n");
	return E_FAIL;
}
...
HRESULT openbor_setdrawmethod(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	if(paramCount >= 15 && varlist[13]->vt != VT_PTR && varlist[13]->vt != VT_EMPTY)
		goto setdrawmethod_error;
	...
	setdrawmethod_error:
	printf
	    ("Function need a valid entity handle and at least 1 interger parameter, all other parameters should be integers: setdrawmethod(entity, int flag, int scalex, int scaley, int flipx, int flipy, int shiftx, int alpha, int remap, int fillcolor, int rotate, int fliprotate, int transparencybg, void* colourmap)\n");
	return E_FAIL;
}
...
HRESULT openbor_performattack(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	*pretvar = NULL;
	...
	performattack_error:
	printf
	    ("Function need a valid entity handle, the other 2 integer parameters are optional: performattack(entity, int anim, int resetable)\n");
	return E_FAIL;
}
...
//		e->stalltime = stalladd;
	...
	setidle_error:
	printf
	    ("Function need a valid entity handle, the other 2 integer parameters are optional: setidle(entity, int anim, int resetable, int stalladd)\n");
	return E_FAIL;
}
...
HRESULT openbor_playgif(ScriptVariant ** varlist, ScriptVariant ** pretvar, int paramCount) {
	...
	(*pretvar)->lVal =
	    (LONG) playgif(StrCache_Get(varlist[0]->strVal), (int) (temp[0]), (int) (temp[1]), (int) (temp[2]));
	...
	playgif_error:
	printf
	    ("Function need a string parameter, other parameters are optional: playgif(path, int x, int y, int noskip)\n");
	*pretvar = NULL;
	return E_FAIL;
}
